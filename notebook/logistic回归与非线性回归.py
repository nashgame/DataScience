...
#参数初始化
filename = '/Volumes/bankloan.xls'
data = pd.read_excel(filename)
x = data.iloc[:,:8].as_matrix()#转换成矩阵
...
plt.scatter(x[0],y)
plt.plot(x[0],np.exp(y_pred),color="blue")
plt.show()
#幂函数
...
plt.scatter(x[0],y)
plt.plot(x[0],np.exp(y_pred),color="red")
plt.show()
